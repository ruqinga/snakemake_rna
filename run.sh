...
if [[ -n "$json_output_pe" ]]; then
    ...
    snakemake \
        --executor cluster-generic \
        --cluster-generic-submit-cmd 'qsub -q slst_pub -N rna_pe.pbs -l nodes=1:ppn=20 -j oe' \
        --latency-wait 60 \
        --jobs 8 \
        --use-conda \
        --group-components processing=8 \
...
if [[ -n "$json_output_se" ]]; then
    ...
    snakemake \
        --executor cluster-generic \
        --cluster-generic-submit-cmd 'qsub -q slst_pub -N rna_se.pbs -l nodes=1:ppn=20 -j oe' \
        --latency-wait 60 \
        --jobs 8 \
        --use-conda \
        --group-components processing=8 \
